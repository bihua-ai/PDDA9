...
interface PulseDistributionChartProps {
  deviceId?: string;
  startTime: string;
  endTime: string;
}
...
export default function PulseDistributionChart({ deviceId, startTime, endTime }: PulseDistributionChartProps) {
  const [data, setData] = useState<ChartData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    console.log('PulseDistributionChart props:', { deviceId, startTime, endTime });
    ...
    const loadChartData = async () => {
      ...
      try {
        ...
        console.log('Received chart data:', {
          ...
          pulse_x_length: chartData.pulse_x?.length,
          pulse_y_length: chartData.pulse_y?.length
        });
        
        if (!chartData.pulse_x?.length || !chartData.pulse_y?.length) {
          console.log('No valid pulse data in response');
          setError('所选时间段内无数据');
          setData(null);
          return;
        }
